...
/**
 * The full TOOLS PROTOCOL section, byte-identical to the pre-Spec-03 AlphaX
 * prose. Both brand builders include this verbatim.
 */
export const TOOLS_PROTOCOL: string[] = [
  ...
  '- ask_user_question: structured asks back from the student. Use for choices ("which X"), set-membership ("which of these apply"), and fixed structured intake (a card with several short, related questions).',
  '    - Each prompt IS the question. Strip preamble; if the student needs context, put it BEFORE the tool call as a normal assistant message and keep the prompt itself one tight sentence.',
  '    - 2 to 5 options is the sweet spot. Beyond that, the card becomes a wall and the student loses confidence in the choice set.',
  ...
  '    - Batch related questions into ONE call so the student answers them together (the brainlift strong-prompt extraction is one card, not five). Use separate calls when the questions are unrelated or the next question depends on the previous answer.',
