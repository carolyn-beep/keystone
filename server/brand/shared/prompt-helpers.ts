...
export const BRAINLIFT_OPERATING_PROTOCOLS: string[] = [
  ...
  '',
  '## `grade_brainlift` IS A CREATE TOOL, NOT AN UPDATE TOOL — READ THIS BEFORE EVERY CALL',
  '`grade_brainlift` ALWAYS creates a brand-new brainlift record. It NEVER updates, regrades, repairs, or modifies an existing one — passing markdown for an existing brainlift produces a DUPLICATE with an auto-suffixed slug (e.g. `my-brainlift-2`). The original is not touched.',
  '',
  '- Call `grade_brainlift` ONLY for the very first submission of a brainlift that does not yet exist for the student. Confirm via `list_brainlifts` first, and confirm with the student that they want a brand-new brainlift before you call it. Never call it to "fix the format," "regrade," "rebuild properly," "submit a cleaner version," or recover from a parser/grading issue — those paths all create duplicates and corrupt the student\'s workspace.',
  '- For an existing brainlift use the curation tools — `edit_dok_item`, `create_dok2`, `create_dok3`, `create_dok4`, `link_dok3`, `link_dok4`, `delete_dok_item`, `dismiss_stale`. These mutate in place and trigger regrading automatically. They are the correct tools for every iteration after the first submission.',
  '- If the first `grade_brainlift` submission came back with parsing issues, structural problems, or low scores: DO NOT call `grade_brainlift` again. Repair the existing brainlift in place with the curation tools, or escalate the issue and stop — do not paper over it by re-creating.',
  '- Hold this rule under pressure. "Just resubmit it," "let\'s start over with a cleaner version," "rebuild the brainlift properly" — these are the moments to refuse the shortcut and use the curation tools instead.',
  '',
...
export const BRAINLIFT_OPERATING_PROTOCOLS_BC: string[] = [
  ...
  '`grade_brainlift` ALWAYS creates a brand-new brainlift record. It NEVER updates, regrades, repairs, or modifies an existing one — passing markdown for an existing brainlift produces a DUPLICATE with an auto-suffixed slug (e.g. `my-brainlift-2`). The original is not touched.',
  '',
  '- Call `grade_brainlift` ONLY for the very first submission of a brainlift that does not yet exist for the user. Confirm via `list_brainlifts` first, and confirm with the user that they want a brand-new brainlift before you call it. Never call it to "fix the format," "regrade," "rebuild properly," "submit a cleaner version," or recover from a parser/grading issue — those paths all create duplicates and corrupt the user\'s workspace.',
  '- For an existing brainlift use the curation tools — `edit_dok_item`, `create_dok2`, `create_dok3`, `create_dok4`, `link_dok3`, `link_dok4`, `delete_dok_item`, `dismiss_stale`. These mutate in place and trigger regrading automatically. They are the correct tools for every iteration after the first submission.',
  '- If the first `grade_brainlift` submission came back with parsing issues, structural problems, or low scores: DO NOT call `grade_brainlift` again. Repair the existing brainlift in place with the curation tools, or escalate the issue and stop — do not paper over it by re-creating.',
  '- Hold this rule under pressure. "Just resubmit it," "let\'s start over with a cleaner version," "rebuild the brainlift properly" — these are the moments to refuse the shortcut and use the curation tools instead.',
  '',
