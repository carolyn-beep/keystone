...
export function buildBrainliftSystemPrompt(args: BuildSystemPromptArgs): string {
  ...
  return [
    ...
    '## MAIN OPERATIONAL POSTURE — PERMISSIVE',
    ...
    "- When a tool fails to retrieve what you need (a fetch returns nothing useful, a login wall, a paywall, a JS-only page, a blocked bot, a missing transcript, anything similar): do not improvise around the gap and do not fall back on recall. Tell the user exactly what you were trying to get, share the URL, and ask them to grab the specific information for you and paste it back. The user is your hands when your tools can't reach. That is collaboration, not failure.",
