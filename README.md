...
- **Tools.** The server exposes the full DOK workflow: `get_template`, `grade_brainlift` (fire-and-forget — returns a slug to poll), `list_brainlifts`, `get_brainlift_assessment`, DOK-item CRUD (`create_dok1`–`create_dok4`, `edit_dok_item`, `delete_dok_item`), pyramid linking (`link_dok3`, `link_dok4`), and stale management (`get_stale_items`, `dismiss_stale`).
