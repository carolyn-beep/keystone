...
5. `grade_brainlift` is the FIRST-SUBMISSION-ONLY tool — it creates a brand-new brainlift and queues it for grading. After that very first submission, NEVER call `grade_brainlift` again on the same topic; doing so creates a duplicate brainlift with an auto-suffixed slug (`my-brainlift-2`) instead of updating the original. To iterate, use `get_brainlift_assessment` (for ALL FOUR DOK LEVELS — 1, 2, 3, AND 4) to read scores and weak items across the full chain, and the curation tools (`edit_dok_item`, `create_dok2`, `create_dok3`, `create_dok4`, `link_dok3`, `link_dok4`, `delete_dok_item`, `dismiss_stale`) to address them in place — these mutate the existing brainlift and trigger regrading automatically.
